...
豆包 App、微信等移动端都有非常好用的语音输入功能，但在 **Windows 桌面端** 却无法直接使用。
Windows 自带的语音输入识别效果经常出错，远不如豆包的识别准确。
